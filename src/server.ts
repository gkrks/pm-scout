...
import { findHiringManager } from "./peopleFinder";
...
<script>
  ...
  function filteredSortedJobs() {
    ...
    var list = allJobs.filter(function(j) {
      ...
      if (newOnly && !isJobNew(j)) return false;
      return true;
    });
...
app.post("/api/people-finder", async (req: Request, res: Response) => {
  const { jobId } = req.body as { jobId?: string };
  if (!jobId) return res.status(400).json({ error: "jobId required" });

  const job = appState.jobs.find((j) => j.id === jobId);
  if (!job) return res.status(404).json({ error: "Job not found — run a scan first" });

  try {
    const result = await findHiringManager(job);
    res.json(result);
  } catch (err) {
...
app.post("/api/apply", (req: Request, res: Response) => {
  const { userId, jobId, email, appliedAt } = req.body as {
    userId: string; jobId: string; email: string; appliedAt?: string;
  };
  ...
  appState.applications[key] = record;
...
app.get("/api/applications", (req: Request, res: Response) => {
  ...
  const records = Object.values(appState.applications).filter(
    (r) => r.userId === userId
  );
...
app.delete("/api/apply/:userId/:jobId", (req: Request, res: Response) => {
  const userId = String(req.params.userId);
  const jobId  = String(req.params.jobId);
  const key = `${userId}::${jobId}`;
  delete appState.applications[key];
