...
import { withPlaywright, launchChromium, LI_UA } from "./jobScraper";

export interface LIPerson {
  name: string;
  title: string;
  url: string;       // canonical linkedin.com/in/... URL
  snippet: string;   // brief bio excerpt from search result
}
 ...
 */
async function searchDDG(company: string, titleQuery: string): Promise<LIPerson[]> {
  return withPlaywright(async () => {
    ...
    try {
      ...
      const q = encodeURIComponent(
        `site:linkedin.com/in "${company}" (${titleQuery})`,
      );
 ...
 */
async function searchLinkedInPeople(company: string, titleQuery: string): Promise<LIPerson[]> {
  return withPlaywright(async () => {
    const browser = await launchChromium();
    try {
      ...
      const people = await page.evaluate((): LIPerson[] => {
        const results: LIPerson[] = [];
        // LinkedIn renders cards in .entity-result__item or similar
        const cards = document.querySelectorAll(
          ".entity-result__item, .search-result, [data-chameleon-result-urn]",
        );
        cards.forEach((card) => {
          const nameEl = card.querySelector(
            ".entity-result__title-text span[aria-hidden='true'], .actor-name, .name",
          );
          const titleEl = card.querySelector(
            ".entity-result__primary-subtitle, .subline-level-1, .search-result__info .subline-level-1",
          );
          const linkEl = card.querySelector("a[href*='linkedin.com/in/']") as HTMLAnchorElement | null;
...
export async function findLinkedInProfiles(
  ...
  if (ddg.status === "rejected") console.warn("[people-finder] DDG search failed:", ddg.reason);
  if (li.status  === "rejected") console.warn("[people-finder] LinkedIn search failed:", li.reason);

  // Merge — deduplicate by LinkedIn slug
  const seen = new Set<string>();
  const merged: LIPerson[] = [];
