...
async function categorizeProfiles(signals, allProfiles, job) {
    if (allProfiles.length === 0) {
        return { candidates: [], eliminated: ["No Apollo profiles returned"] };
    }
    const profileList = allProfiles.map((p, i) => `${i + 1}. Name: ${p.name}\n   Title: ${p.title}\n   URL: ${p.linkedInUrl || ""}\n   Org: ${p.organization}`).join("\n\n");
    const completion = await client.chat.completions.create({
        model: MODEL,
        max_tokens: 2000,
        temperature: 0,
...
function liSearch(company, keywords) {
...
async function findHiringManager(job) {
    ...
    const [hmProfiles, recProfiles, peerProfiles] = await Promise.all([
        (0, apolloClient_1.searchApollo)(job.company, {
    ...
    console.log(`[people-finder] Pass 2 done — ${hmProfiles.length} HMs, ` +
